...
#Generate Fold
GenerateFold('./case '+str(case))
GenerateFold('./case '+str(case)+'/input')

GenerateFold('./case '+str(case)+'/input/cumulative strain')
GenerateFold('./case '+str(case)+'/input/periodical strain')
GenerateFold('./case '+str(case)+'/input/stress')
...
while len(yade_rgb_list)<n_layer:
...
#detachment index
k_detachment=np.round(n_layer/2)
...
for i in spheres:
	...
	for k in range(n_layer):
		...
		if k*height_step<=O.bodies[i].state.pos[1]<=(k+1)*height_step:
			...
			#case1+
			if k_detachment-(case-1)<=k<=k_detachment+(case-1):
		
				O.bodies[i].shape.color = yade_rgb_list[k_detachment]
...
out_file=open('./case '+str(case)+'/input/stress/progress='+'%.2f%%' %progress+".txt",'w')
...
#init
TW1.setState(1)
TW1.defToVtk('./case '+str(case)+'/input/cumulative strain/progress='+'%.2f%%' %progress+".vtk")

TW2.setState(1)
TW2.defToVtk('./case '+str(case)+'/input/periodical strain/progress='+'%.2f%%' %progress+".vtk")
