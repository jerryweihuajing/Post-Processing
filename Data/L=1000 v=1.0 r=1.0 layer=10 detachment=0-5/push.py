...
#Generate Fold
GenerateFold('./input/case '+str(case))
...
for i in spheres:
	...
	for k in range(n_layer):
		...
		#case1+
		if 0.5*(maxh-case*height_step)<=O.bodies[i].state.pos[1]<=0.5*(maxh+case*height_step):
...
out_file=open('./input/case '+str(case)+'/stress/progress='+'%.2f%%' %progress+".txt",'w')
